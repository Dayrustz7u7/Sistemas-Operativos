...
void filtrador(int pipeIzquierdo[]){
	...
	if( pideSecuenciales == 0){ //SON LOS HIJOS QUE SE ENCARGARA DE CREAR SU PROPIO PIPE Y VERIFICACION
		...
		while (read(pipeIzquierdo[READ], &candidatoAPrimo, sizeof(candidatoAPrimo)) != 0) {
			if (candidatoAPrimo % primo != 0) {
				write(pipeDerecho[WRITE], &candidatoAPrimo, sizeof(candidatoAPrimo));
			}
		}
		close(pipeDerecho[WRITE]);
		close(pipeIzquierdo[WRITE]);
